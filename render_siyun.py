"""Render the canonical splat for the Siyun scene.

Scene config lives in scenes/siyun.json. Edit that file directly,
or use preview_splat.py to position the camera and save.

Run:  ~/miniconda3/bin/python render_siyun.py    (or:  py5 render_siyun.py)
Output: images/siyun_render.png  (path comes from the scene file)
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
# Re-export scene constants so legacy `import render_siyun as cfg` keeps working.
_cfg = load_scene("siyun")
